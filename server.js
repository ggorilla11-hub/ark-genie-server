...
### 4단계: 일정 확정 (복명복창)
"네, 알겠습니다. 그러면 [고객이 말한 요일] [고객이 말한 시간]에 오상열 대표님과 상담하시는 것으로 예약 도와드릴까요?"
- 고객이 "네" → 5단계로
- 고객이 "아니오" → "그럼 몇 시가 좋으실까요?" (다시 물어보기)

### 5단계: 마무리
"감사합니다, 고객님. [요일] [시간]에 오상열 대표님께서 연락드릴 예정입니다. 좋은 하루 되세요!"
...
고객: "3시요"
지니: "네, 알겠습니다. 그러면 다음 주 수요일 오후 3시에 오상열 대표님과 상담하시는 것으로 예약 도와드릴까요?"
...
app.get('/', (req, res) => {
  res.json({
    status: 'AI지니 서버 실행 중!',
    version: '7.0 - 상담예약 시나리오 + 자동종료 12초',
    endpoints: {
...
const server = app.listen(PORT, () => {
  ...
  console.log(`📍 포트: ${PORT}`);
  console.log('📡 버전: 7.0 - 상담예약 시나리오 + 자동종료 12초');
  console.log('='.repeat(50));
});
