// ============================================
// ARK-Genie Server v21.2 - 담보 표 우선 추출 OCR
// - 🆕 담보/특약 표를 1순위로 추출
// - 🆕 주계약 + 특약 정확한 금액 추출
// - 🆕 보장 요약 (사망/장해/기타)
// - 🆕 기본정보는 간략히 (마지막)
...
app.post('/api/analyze-image', async (req, res) => {
  try {
    ...
    const expertPrompt = prompt || `당신은 보험증권 OCR 전문가입니다. 

## 🚨 최우선 작업: 담보/특약 표 추출

보험증권에서 **네모 박스 안의 담보 표**를 가장 먼저, 가장 정확하게 추출하세요.
다른 정보는 나중에! 담보 표가 가장 중요합니다!

### 📊 담보 표 읽는 법
- 표 컬럼: 구분 | 보험가입금액 | 보험기간 | 납기 | 보험료(월) | 기간 및 합계
- **보험가입금액** = 보장받는 금액 (만원 단위, 예: 7,000만원)
- **보험료** = 내는 돈 (원 단위, 예: 96,600원)
- **주계약** = "특약"이 안 붙은 것 (예: VVUL평준형 보험, 종신보험 등)
- **특약** = "특약"이 붙은 것 (예: 재해사망특약, 수술특약 등)
...
#### 주계약
| 담보명 | 보험가입금액 | 보험기간 | 월보험료 |
|--------|-------------|---------|---------|
| (정확한 상품명) | O,OOO만원 | OO년/종신 | OO,OOO원 |
...
## ⚠️ OCR 주의사항
1. 담보 표의 숫자를 정확히 읽을 것
2. "7,000만원"을 "96,600원"으로 혼동하지 말 것
3. 표가 여러 개면 모두 추출
4. 읽기 어려운 부분은 "불명확"으로 표시

## 📋 기타 문서
- 의료비 영수증: 병원명, 진료비, 본인부담금, 실손청구 가능여부
- 명함: 이름, 직책, 회사, 연락처`;
